/**
 * A class that displays a repeating texture that can optionally be offset on the x or y axis
 * @author Jacob Jamell
 * @class
 */
function RepeatingGameObject()
{
	/** The width the final image will take up
	 * @type Number
	 */
	this.width = 0;
	...
	this.draw = function(dt, canvas, xScroll, yScroll)
	{
		...
		for (var y = 0; y < this.height; y += areaDrawn[1])
		{
			for (var x = 0; x < this.width; x += areaDrawn[0])
			{
				...
				// the first time around you have to start drawing from the middle
				//subsequent tiles always get drawn from the top or left
